...
```
Client (TCP :5555)
    │  plain-text line: "JOIN player-1 world-1\n"
    ▼
TcpServer
...
Command implementations (JoinCommand, MoveCommand, ...)
    ...
    ▼
ServerInstance  (one per world)
    │  delegates to CommandActor
    ▼
CommandActor  (single-threaded executor per world, named "actor-<worldName>-ctl")
    │  join()   → WorldConnector.apply(JoinOp)
    │  stageMove() → ConcurrentHashMap<playerId, Direction>  [last-write-wins]
    │  step()   → applyMoves() + world.update() + onAfterUpdate.run()
    ▼
...
| `model.world.engine` | `GridWorldEngine` (tick logic), `OpenWorldEngine` (stub) |
| `model.world.state` | `GridWorldState` (occupancy, entity map) |
| `model.world.connector` | `WorldConnector` interface; `GridWorldConnector`, `TrafficConnector` (stub), `OpenWorldConnector` (stub) |
| `model.world.protocol` | `CommandType`, `Message`, `StateFrame`, `JsonProtocolEncoder`, `ServerResponse`, `CommandResult` |
...
| `ServerBootstrap.java` | creates worlds, clocks, instances, starts `TcpServer` |
...
`shutdown()` → `cleanupResources()`: if the session was authenticated and joined a world, calls `instance.leaveAsync(session)` on the actor thread, then closes the connection.
...
- Single-player world busy check (`maxPlayers == 1`)
- 750 ms timeout on the join future

### 3.4 Actor (`CommandActor`)

Single-threaded `ExecutorService` named `actor-<worldName>-ctl`. All world mutations are serialized through it.

- **`stageMove(world, req)`** — does NOT submit to the executor; writes `playerId → Direction` into a `ConcurrentHashMap` directly and returns an already-completed future. Moves are coalesced (last-write-wins).
- **`step(world, onAfterUpdate)`** — submits to executor: flush all staged moves, `world.update()`, then calls `onAfterUpdate` (which is `broadcastWorldSnapshot()`). Broadcast therefore runs **on the actor thread** — noted with `// TODO: Broadcast off-actor thread` in `ServerInstance`.
...
Owns the per-world lifecycle. Extends `AbstractInstance`.

- `start()` — begins clock ticking; idempotent via `clockTicking` CAS.
- `stop()` — 7-step graceful shutdown: stop clock → drain actor → shutdown sessions → drain again → reset world → stop actor → shutdown clock.
...
- `broadcastWorldSnapshot()` — calls `world.snapshot()`, encodes to JSON, publishes to all sessions. Runs on actor thread (see §3.4 TODO).
...
- `update()` — advance the world by one tick
- `snapshot()` — capture current state for broadcast
- `reset()` — wipe world state on instance stop
...
```json
{"version":1,"worldName":"world-1","seq":42,"data":{ ...world snapshot... }}
```
...
Tick rate (120), broadcast rate (70), port (5555), and world names are all literals in `ServerBootstrap`. No config file, no env-variable overrides.
...
**`flags` byte future-use note.** Listing `0x02 = ack required` as a future flag implies a reliability layer. For a game server sending world snapshots, lost frames are acceptable (client re-syncs on next tick). Implementing ACK required would add round-trip overhead on the hot path. If the flag is ever used, it should only apply to transactional messages (JOIN, AUTH), not state pushes.
...
| 12 | Externalize configuration | `application.properties`: port, tick rate, broadcast rate, world definitions |
...
- **Actor-per-world** — correct serialization model; do not add intra-world parallelism.
